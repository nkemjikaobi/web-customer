...
interface ITravellersFieldComponent {
  ageRange: string;
...
const TravellersFieldComponent: React.FunctionComponent<
  ITravellersFieldComponent
> = (props: ITravellersFieldComponent) => {
  const [title, setTitle] = useState<string>("");
  const [ageRange, setAgeRange] = useState<string>("");
  const [counter, setCounter] = useState<number>(0);

  useEffect(() => {
    let mounted: ITravellersFieldComponent | null = props;
    if (mounted) {
...
export default TravellersFieldComponent;
