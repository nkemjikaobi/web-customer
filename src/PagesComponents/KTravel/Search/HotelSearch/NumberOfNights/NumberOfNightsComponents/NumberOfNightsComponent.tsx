...
interface INumberOfNightsComponent {
  ageRange: string;
...
const NumberOfNightsComponent: React.FunctionComponent<
  INumberOfNightsComponent
> = (props: INumberOfNightsComponent) => {
  const [title, setTitle] = useState<string>("");
  const [ageRange, setAgeRange] = useState<string>("");
  const [counter, setCounter] = useState<number>(0);

  useEffect(() => {
    let mounted: INumberOfNightsComponent | null = props;
    if (mounted) {
...
export default NumberOfNightsComponent;
